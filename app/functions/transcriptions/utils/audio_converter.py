...
async def convert_and_store_raw_to_wav_with_ffmpeg_async(
    audio_url: str, sample_rate: int = 8000
):
    """
...
async def convert_and_segment_audio_async(
    ...
    try:
        ...
        file_size_mb = await asyncio.to_thread(os.path.getsize, wav_file_path) / (
            1024 * 1024
        )
        ...
        segment_paths = await split_audio_into_segments(
            wav_file_path, num_segments, duration
        )
...
async def split_audio_into_segments(
    wav_file_path: str, num_segments: int, total_duration: float
) -> List[str]:
    """
    ...
    try:
        ...
        if sample_rate == 8000:
            ...
            ffmpeg_cmd = [
                ...
                "-ar",
                "8000",  # Explicit sample rate: 8kHz
                "-ac",
                "1",  # Explicit channels: mono
                "-i",
                temp_input.name,  # Input from pipe
                "-acodec",
                "pcm_s16le",
                "-ar",
            ...
            ffmpeg_cmd = [
                ...
                "s16le",  # Explicit format: 16-bit signed PCM
                "-ar",
                "16000",  # Explicit sample rate: 16kHz
                "-ac",
                "1",  # Explicit channels: mono
                "-i",
                temp_input.name,  # Input from pipe
            ...
            ffmpeg_cmd = [
                "ffmpeg",
                "-i",
                temp_input.name,  # Auto-detect input format
                "-acodec",
                "pcm_s16le",
                "-ar",
                "16000",  # Output sample rate: 16kHz
