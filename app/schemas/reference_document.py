...
class ReferenceDocumentCreate(BaseModel):
    ...
    tags: List[str] = Field(default_factory=list, description="Tags associated with the reference document")
    tenant_id: str = Field(..., description="Tenant ID associated with the reference document")
...
class ReferenceDocumentUpdate(BaseModel):
    ...
    heading: Optional[str] = Field(None, description="Updated heading or title of the reference document")
    content: Optional[str] = Field(None, description="Updated content of the reference document")
    category: Optional[str] = Field(None, description="Updated category of the reference document")
    tags: Optional[List[str]] = Field(None, description="Updated tags associated with the reference document")
...
class ReferenceDocumentResponse(BaseModel):
    ...
    content: str = Field(..., description="Content of the reference document")
    category: str = Field(..., description="Category of the reference document")
    tags: List[str] = Field(..., description="Tags associated with the reference document")
    tenant_id: str = Field(..., description="Tenant ID associated with the reference document")
...
class ReferenceDocumentSearchRequest(BaseModel):
    ...
    document_ids: Optional[List[str]] = Field(None, description="List of document IDs to restrict the search to")
    limit: int = Field(10, description="Maximum number of results to return")
    filters: Optional[Dict[str, Any]] = Field(None, description="Filters to apply to the search (supported keys: category, tags, tenant_id)")
    sort_by: Optional[str] = Field(None, description="Field to sort by (e.g., 'heading', 'category')")
