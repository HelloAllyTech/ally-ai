...
class RoadmapOpportunityBulkUpsertResponse(BaseModel):
    ...
    A batch endpoint that reports overall success while silently dropping items is
    exactly how
    the standalone roadmap app's own backfill wrote 241 fallback classifications to a
    file
...
class RoadmapOpportunitySearchRequest(BaseModel):
    description: str = Field(
        ..., min_length=1, description="Draft text to match against"
    )
    ...
    threshold: float = Field(
        ...
        le=1.0,
        description=(
            "Minimum cosine SIMILARITY (not distance) for a candidate. The standalone "
            "app used "
            "0.5 against Voyage voyage-3-large; re-calibrate for "
...
class RoadmapOpportunityDeleteResponse(BaseModel):
    ...
    deleted: bool = Field(
        ...,
        description=(
            "True when the index no longer holds this id, including when it "
            "never did — "
            ""
...
class RoadmapOpportunityIdsResponse(BaseModel):
    ...
    `next_cursor` is None when the page was not full, i.e. the end of the collection.
    Paging is
    by CURSOR rather than offset because offset paging over a collection being written
    to can skip
    objects — and a sweep that skips an id would under-report drift while looking like
    it passed.
    """
