...
async def create_reference_document(
        ...
        reference_document_service: ReferenceDocumentService = Depends(get_reference_document_service)
):
    """
...
async def delete_reference_document(
    ...
    try:
        ...
        return ReferenceDocumentDeleteResponse(
            success=True
        )
...
async def search_reference_documents(
    ...
    using the filters dictionary with supported keys: 'category', 'tags', and 'tenant_id'.
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_503_SERVICE_UNAVAILABLE,
            detail="Failed to generate embedding for search query. Please try again later."
        )
