...
Same contract as the other judges: ally-be owns the data, picks which turns to judge,
pairs each recall decision with the turn it belongs to out of its own transcript, and
persists the labels. This service only judges.
...
class RecallQualityRequest(BaseModel):
    ...
    # The turn itself. Supplied by ally-be from scenario_session_messages — this
    # service does not look it up, and MUST not: pairing a recall decision with
    # the wrong turn would produce a confident verdict about a conversation that
    # never happened.
    counsellor_turn: str
    client_reply: str
    # Each: {text, score, cue_hits, similarity, ...}. Recalled facts and the near
    # misses.
    selected: List[dict] = Field(default_factory=list)
    passed_over: List[dict] = Field(default_factory=list)
    # The stance recall was scored against. A guarded client withholding a fact is
    # not a recall failure, and a judge that cannot see the stance would read it
    # as one.
...
class RecallQualityResponse(BaseModel):
    ...
    # None only when the judge failed, or when there was no pool to rank. Never a
    # way of saying "this turn needed nothing" — that is `no_demand`, a real
    # verdict, and it is stored as one.
...
async def judge(request: RecallQualityRequest) -> RecallQualityResponse:
    ...
    if not request.counsellor_turn.strip() and not request.client_reply.strip():
        # Neither half of the turn means nothing to judge against. Refused rather
        # than answered, because a verdict on an empty turn is a verdict about
        # nothing.
        raise HTTPException(
