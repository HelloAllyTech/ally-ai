...
class WeaviateDB(VectorDB):
    ...
    async def create_document(self, collection_name: str, document_data: Dict[str, Any], vector: List[float],
                              document_id: str) -> str:
        """
        ...
        try:
                ...
                if "id" in filters:
                    ...
                    if isinstance(doc_ids, list):
                        if len(doc_ids) == 1:
                            ...
                            id_conditions = [Filter.by_id().equal(doc_id) for doc_id in doc_ids]
                ...
                if filters:
                    ...
                    for key, value in filters.items():
                        if isinstance(value, list):
                            ...
                            or_conditions = [Filter.by_property(key).equal(item) for item in value]
                            if or_conditions:
                                # For a single item, just add it directly
                                if len(or_conditions) == 1:
                                    filter_conditions.append(or_conditions[0])
                                else:
                                    # For multiple items, use a list of conditions with Filter.any_of
                                    filter_conditions.append(Filter.any_of(or_conditions))
                        else:
                            # For single values
                    ...
                    if filter_conditions:
                        property_filter = filter_conditions[0] if len(filter_conditions) == 1 else Filter.all_of(
                            filter_conditions)

                        # Combine with ID filter if it exists
                        if query_filters:
                            query_filters = Filter.all_of([query_filters, property_filter])
                        else:
            ...
            for obj in result.objects:
                ...
                if obj.metadata and obj.metadata.distance is not None:
                    document["score"] = 1.0 - obj.metadata.distance  # Convert distance to similarity score
                else:
                    document["score"] = None
