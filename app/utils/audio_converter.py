...
async def convert_and_store_raw_to_wav_with_ffmpeg_async(audio_url: str, sample_rate: int = 8000):
    ...
    try:
        if sample_rate == 8000:
            ...
            return await _convert_with_pipe_streaming(audio_url, frequency=8000)
        elif sample_rate == 16000:
            # 16kHz microphone audio - use explicit format specification
            logger.info("Using 16kHz pipe streaming with explicit format")
            return await _convert_with_pipe_streaming(audio_url, frequency=16000)
        else:
            ...
            return await _convert_with_pipe_streaming(audio_url, frequency=48000)
...
async def _convert_with_pipe_streaming(audio_url: str, frequency: int = 8000) -> str:
