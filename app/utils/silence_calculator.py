...
def calculate_silence_by_counselor(chat_messages: List[ChatMessage]) -> int:
    ...
    Silence is defined as a time frame during which neither the counselor nor
    client has spoken.
    ...
    Parameters:
        chat_messages (List[ChatMessage]): List of chat messages with role,
        content, start_time, and end_time
    ...
    if len(sorted_messages) < 2:
        logger.debug(
            "Not enough messages with valid timestamps for silence calculation"
        )
        return 0
    ...
    for i in range(len(sorted_messages) - 1):
        ...
        if (
            current_msg.start_time is None
            or current_msg.end_time is None
            or next_msg.start_time is None
            or next_msg.end_time is None
        ):
            continue
        ...
        if (
            current_msg.role.lower() == "client"
            and next_msg.role.lower() == "counselor"
        ):
            silence_count += 1
            logger.debug("Adding silence moment (client → counselor)")

        # Check if silence is between two client messages
        elif current_msg.role.lower() == "client" and next_msg.role.lower() == "client":
            silence_count += 1
            logger.debug("Adding silence moment (client → client)")
