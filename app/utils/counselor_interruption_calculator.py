...
def calculate_counselor_interruptions(chat_messages: List[ChatMessage]) -> int:
    ...
    An interruption is defined as a counselor message that starts during a
    client's speaking time, i.e., when a counselor's message start_time falls
    within a client's message start_time to end_time range.

    This implementation uses a sort and linear scan approach for efficiency:
    1. Filter messages with valid timestamps
    2. Sort all messages by start_time (with client messages prioritized in
    case of ties)
    3. Track active client messages as we scan through the sorted list
    4. Count interruptions when a counselor message starts while a client message
    is active

    Parameters:
        chat_messages (List[ChatMessage]): List of chat messages with role, content,
    ...
    valid_messages = [
        ...
        if msg.start_time is not None and msg.end_time is not None
    ]

    if len(valid_messages) < 2:
        logger.debug(
            "Not enough messages with valid timestamps for interruption calculation"
        )
        return 0
        ...
        if msg.role.lower() == "client":
            ...
            for client_msg in active_client_messages:
                if client_msg.start_time < msg.start_time < client_msg.end_time:
                    ...
                    logger.debug(
                        f"Detected counselor interruption at time {msg.start_time}"
                    )
