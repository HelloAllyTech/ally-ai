...
#: Most of these tests are about indexing, scope and staleness rather than access, so
#: they search as the admin console does. Audience behaviour has its own classes.
...
class TestKnowledgeChunkService:
    ...
    async def test_search_with_empty_scope_returns_nothing(
        ...
        assert await service.search("q", ANY_AUDIENCE, limit=2, document_ids=[]) == []
    ...
    async def test_search_combines_language_and_document_scope(
        ...
        await service.search(
            "q", ANY_AUDIENCE, limit=3, document_ids=[DOC_ID], language="ml"
        )
...
class TestCollectionBinding:
    ...
    async def test_every_read_and_write_goes_to_the_bound_collection(
        ...
        await service.search("q", ANY_AUDIENCE)
