...
class TestOpenAITextGenerationService:
    ...
    async def test_generate_simulation_summary_success(
        ...
        """Test successful simulation summary generation (need_memory=False)."""
    ...
    async def test_generate_simulation_summary_with_memory_success(
        ...
        with patch.object(
            ...
            # Assert single LLM call
            mock_llm.assert_called_once()

            # Assert all 4 fields present
            assert result == {
                "improvements": ["Improve reflective listening"],
                "positives": ["Good empathy"],
                "session_glimpse": "Client discussed work stress.",
    ...
    async def test_generate_simulation_summary_with_memory_uses_combined_model(
        ...
        with patch.object(
            ...
            assert call_args[0][1] is SimulationAnalysisWithMemory

    @pytest.mark.asyncio
    async def test_generate_simulation_summary_without_memory_uses_base_model(
        self, text_generation_service, sample_chat_messages
    ):
        """When need_memory=False, _invoke_llm is called with SimulationAnalysis."""
        mock_response = SimulationAnalysis(
            improvements=[],
            positives=[],
        )
        with patch.object(
            ...
            await text_generation_service.generate_simulation_summary(
                sample_chat_messages, need_memory=False
            )

            call_args = mock_llm.call_args
            assert call_args[0][1] is SimulationAnalysis
    ...
    @pytest.mark.asyncio
    async def test_generate_simulation_summary_with_memory_failed(
        self, text_generation_service, sample_chat_messages
    ):
        """Test simulation summary with memory generation failure."""
        with patch.object(
            text_generation_service,
            "_invoke_llm",
            side_effect=LLMInvocationFailedException("LLM error"),
