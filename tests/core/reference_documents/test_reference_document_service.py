...
class TestSearchIsReported:
    ...
    @pytest.fixture
    def service(self):
        return ReferenceDocumentService(AsyncMock(), AsyncMock())

    @pytest.mark.asyncio
    async def test_reports_the_search_with_the_floor_converted_from_distance(
        self, service
    ):
        service.vector_db.search_documents.return_value = {
            "documents": [
                {"id": "doc-1", "heading": "h", "content": "c", "score": 0.81},
                {"id": "doc-2", "heading": "h", "content": "c", "score": 0.44},
            ],
            "total": 2,
            "categories": ["test"],
        }
    ...
    @pytest.mark.asyncio
    async def test_marks_the_query_sensitive(self, service):
        service.vector_db.search_documents.return_value = {
            "documents": [],
            "total": 0,
            "categories": [],
    ...
    async def test_reports_a_whole_document_as_the_unit_of_retrieval(self, service):
        ...
        hit = emit.call_args.kwargs["hits"][0]
        assert hit["chunk_id"] == "doc-1"
